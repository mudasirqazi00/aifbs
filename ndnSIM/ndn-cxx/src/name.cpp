...
//  LogManager::AddLog("name.cpp->compare.this.name", this->toUri());
//  LogManager::AddLog("name.cpp->compare.other.name", other.toUri());

  for (size_t i = 0; i < count; ++i) {
    int comp = get(pos1 + i).compare(other.get(pos2 + i));
    if (comp != 0) { // i-th component differs
//	  LogManager::AddLog("name.cpp->compare.comp", comp);
      return comp;
    }
  }

//  LogManager::AddLogWithNodeId("name.cpp->compare.return", (int)(count1 - count2));
