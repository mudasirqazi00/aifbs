...
public:
	 ...
	 */
	explicit
	Interest(const Name& name = Name(), time::milliseconds interestLifetime =
	...
	/**
	 * @brief Separate method to wireEncode m_accNames
	 */
	template<encoding::Tag TAG>
	size_t
	wireEncodeAccNames(EncodingImpl<TAG>& encoder) const;
	...
	/**
	 * @brief Decode m_accNames from the wire format
	 */
	std::vector<Name>
	wireDecodeAccNames(const Block& wire);
...
public:
	Interest&
	setPreviousNodeId(const uint32_t id);

	uint32_t
	getPreviousNodeId() const;

	Interest&
	setAccNames(const std::vector<Name> v);

	std::vector<Name>
	getAccNames() const;

/*
	void setPreviousNodeId(const uint32_t id) {
		m_previousNodeId = id;
	}
	uint32_t getPreviousNodeId() const {
		return m_previousNodeId;
	}
	void setAccNames(const std::vector<Name> v) {
		m_accNames = v;
	}
	std::vector<Name> getAccNames() const {
		return m_accNames;
	}
*/
