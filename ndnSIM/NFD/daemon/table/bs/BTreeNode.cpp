...
namespace nfd {
	namespace cs {
		...
		int BTreeNode<T>::size() {
			int size = 0;
//			LogManager::AddLogWithNodeId("BTreeNode.cpp->size.size", size);
//			LogManager::AddLogWithNodeId("BTreeNode.cpp->size.n", n);
			int i;
			for (i = 0; i < n; i++) {//value of n is wrong
//				LogManager::AddLogWithNodeId("BTreeNode.cpp->size.loop.i", i);
//				LogManager::AddLogWithNodeId("BTreeNode.cpp->size.loop.size", size);
				size += C[i]->size();
			}
//			LogManager::AddLogWithNodeId("BTreeNode.cpp->size.complete.size", size);
			return size;
		}
		...
		T *BTreeNode<T>::searchKeyRef(T k)
		{
			try
			  {
//			LogManager::AddLog("btreenode.cpp->searchKeyRef");
			// Find the first key greater than or equal to k
			int i = 0;
			while (i < n && k > keys[i]){
				i++;
			...
			// If the found key is equal to k, return this node
			if (keys[i] == k){
//				LogManager::AddLog("btreenode.cpp->searchKeyRef.return");
				return &keys[i];
			}
...
//			LogManager::AddLog("btreenode.cpp->searchKeyRef.C[i]->searchKeyRef(k)");
			// Go to the appropriate child
			return C[i]->searchKeyRef(k);
			  }
			catch (exception& e) {
//				LogManager::AddLog("btreenode.cpp->searchKeyRef.exception", e.what());
				return NULL;
			}
		}
